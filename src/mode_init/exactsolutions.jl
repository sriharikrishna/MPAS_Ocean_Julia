function kelvinWaveGenerator(mpasOcean, lateralProfile)
    ...
    function kelvinWaveExactNormalVelocity(mpasOcean, iEdge, t = 0)
        v =
            c * lateralProfile.((mpasOcean.yEdge[iEdge] .+ c * t) .% lYEdge) .*
            exp.(-mpasOcean.xEdge[iEdge] / rossbyRadiusR)
        return v .* sin.(mpasOcean.angleEdge[iEdge])
    end

    function kelvinWaveExactSSH(mpasOcean, iCell, t = 0)
        return -meanFluidThicknessH *
               lateralProfile.((mpasOcean.yCell[iCell] .+ c * t) .% lYEdge) .*
               exp.(-mpasOcean.xCell[iCell] / rossbyRadiusR)
    end

    function kelvinWaveExactSolution!(mpasOcean, t = 0)
        # just calculate exact solution once then copy it to lower layers
        sshperlayer =
            kelvinWaveExactSSH(mpasOcean, collect(1:mpasOcean.nCells), t) /
        ...
        nvperlayer =
            kelvinWaveExactNormalVelocity(mpasOcean, collect(1:mpasOcean.nEdges), t) /
            mpasOcean.nVertLevels
        for k = 1:mpasOcean.nVertLevels
    ...
    function boundaryCondition!(mpasOcean, t)
        for iEdge = 1:mpasOcean.nEdges
            if mpasOcean.boundaryEdge[iEdge] == 1.0
                mpasOcean.normalVelocityCurrent[:, iEdge] .=
                    kelvinWaveExactNormalVelocity(mpasOcean, iEdge, t) /
                    mpasOcean.nVertLevels
            end
        end
    ...
    return kelvinWaveExactNormalVelocity,
    kelvinWaveExactSSH,
...
function inertiaGravityExactSolution!(
    ...
    for iCell = 1:mpasOcean.nCells
        ...
        mpasOcean.layerThickness[:, iCell] .+=
            DetermineInertiaGravityWaveExactSurfaceElevation(
                etaHat,
                kX,
                kY,
                omega,
    ...
    for iEdge = 1:mpasOcean.nEdges
        ...
        u = DetermineInertiaGravityWaveExactZonalVelocity(
            etaHat,
            f0,
            mpasOcean.gravity,
            kX,
        ...
        v = DetermineInertiaGravityWaveExactMeridionalVelocity(
            etaHat,
            f0,
            mpasOcean.gravity,
            kX,
        ...
        mpasOcean.normalVelocityCurrent[:, iEdge] .=
            (u * cos(theta) + v * sin(theta)) / mpasOcean.nVertLevels
    end
end
...
function DetermineInertiaGravityWaveExactMeridionalVelocity(
    etaHat,
    f0,
    g,
    kX,
    ...
    v =
        etaHat * (
            g / (omega^2.0 - f0^2.0) * (
                omega * kY * cos(kX * x + kY * y - omega * time) +
                f0 * kX * sin(kX * x + kY * y - omega * time)
            )
        )
    return v
end
function DetermineInertiaGravityWaveExactZonalVelocity(
    etaHat,
    f0,
    g,
    kX,
    ...
    u =
        etaHat * (
            g / (omega^2.0 - f0^2.0) * (
                omega * kX * cos(kX * x + kY * y - omega * time) -
