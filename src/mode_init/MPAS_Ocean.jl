...
function MPAS_Ocean{FT}(
    mesh_directory = "MPAS_O_Shallow_Water/Mesh+Initial_Condition+Registry_Files/Periodic",
    ...
    #mpasOcean = new{FT}()
    base_mesh_file =
        NCDatasets.Dataset("$mesh_directory/$base_mesh_file_name", "r", format = :netcdf4)
    mesh_file =
        NCDatasets.Dataset("$mesh_directory/$mesh_file_name", "r", format = :netcdf4)
    ...
    for iVertex = 1:_nVertices
        ...
        for Int64 = 1:_vertexDegree
            ...
            if iCell == 0
                ...
                _maxLevelVertexBot[iVertex] =
                    max(_maxLevelVertexBot[iVertex], _maxLevelCell[iCell])
                _maxLevelVertexTop[iVertex] =
                    min(_maxLevelVertexTop[iVertex], _maxLevelCell[iCell])
            end
        end
    end
...
function ocn_init_routines_compute_max_level!(mpasOcean)
    ...
    for iEdge = 1:mpasOcean.nEdges
        ...
        elseif !(iCell1 == 0 || iCell2 == 0)
            mpasOcean.maxLevelEdgeTop[iEdge] =
                min(mpasOcean.maxLevelCell[iCell1], mpasOcean.maxLevelCell[iCell2])
            mpasOcean.maxLevelEdgeBot[iEdge] =
                max(mpasOcean.maxLevelCell[iCell1], mpasOcean.maxLevelCell[iCell2])
        end
    end
    ...
    for iVertex = 1:mpasOcean.nVertices
        ...
        for Int64 = 1:mpasOcean.vertexDegree
            iCell = mpasOcean.cellsOnVertex[Int64, iVertex]
            if iCell == 0
                mpasOcean.maxLevelVertexBot[iVertex] =
                    max(mpasOcean.maxLevelVertexBot[iVertex], -1)
                mpasOcean.maxLevelVertexBot[iVertex] =
                    min(mpasOcean.maxLevelVertexTop[iVertex], -1)
            else
                mpasOcean.maxLevelVertexBot[iVertex] =
                    max(mpasOcean.maxLevelVertexBot[iVertex], mpasOcean.maxLevelCell[iCell])
                mpasOcean.maxLevelVertexTop[iVertex] =
                    min(mpasOcean.maxLevelVertexTop[iVertex], mpasOcean.maxLevelCell[iCell])
            end
        end
    end
