...
function calculate_normal_velocity_tendency!(mpasOcean::MPAS_Ocean)
    ...
    calculate_normal_velocity_tendency_kernel!(mpasOcean.backend)(
        mpasOcean.nEdges,
...
@kernel function calculate_normal_velocity_tendency_kernel!(
    nEdges,
    ...
    if iEdge <= nEdges
        ...
        for k = 1:maxLevelEdgeTop[iEdge]
            normalVelocityTendency[k, iEdge] =
                gravity * (layerThickness[k, cell1] - layerThickness[k, cell2]) /
                dcEdge[iEdge]
        end
        ...
        for i = 1:nEdgesOnEdge[iEdge]
            ...
            if eoe != 0
                for k = 1:maxLevelEdgeTop[iEdge]
                    normalVelocityTendency[k, iEdge] +=
                        weightsOnEdge[i, iEdge] * normalVelocity[k, eoe] * fEdge[eoe]
                end
            end
        end
...
function update_normal_velocity_by_tendency!(mpasOcean::MPAS_Ocean)
    ...
    update_normal_velocity_by_tendency_kernel!(mpasOcean.backend)(
        mpasOcean.nEdges,
        mpasOcean.normalVelocityCurrent,
        mpasOcean.dt,
        mpasOcean.normalVelocityTendency,
...
@kernel function update_normal_velocity_by_tendency_kernel!(
    nEdges,
...
function calculate_thickness_tendency!(mpasOcean::MPAS_Ocean)
    ...
    calculate_thickness_tendency_kernel!(mpasOcean.backend)(
        mpasOcean.nCells,
...
@kernel function calculate_thickness_tendency_kernel!(
    nCells,
    ...
    if iCell <= nCells
        ...
        for i = 1:nEdgesOnCell[iCell]
            iEdge = edgesOnCell[i, iCell]
            iCell2 = cellsOnCell[i, iCell]

            for k = 1:maxLevelEdgeTop[iEdge]
                layerThicknessEdge =
                    0.5 * (layerThickness[k, iCell2] + layerThickness[k, iCell])
                layerThicknessTendency[k, iCell] +=
                    normalVelocity[k, iEdge] *
                    edgeSignOnCell[iCell, i] *
                    dvEdge[iEdge] *
                    layerThicknessEdge / areaCell[iCell]
            end
        end
...
@kernel function update_thickness_by_tendency_kernel!(
    nCells,
    layerThickness,
    dt,
    layerThicknessTendency,
